export const calculateTimeDifference = (tweetCreatedAt: string): string => {
    ...
    const timeDifferenceInMilliseconds: number = currentTime.getTime() - tweetCreatedAtTime.getTime();
    ...
    if (timeDifferenceInMilliseconds < minutesInMs) {
        const seconds: number = Math.floor(timeDifferenceInMilliseconds / secondsInMs);
        return `${seconds} second${seconds !== 1 ? 's' : ''} ago`;
    } else if (timeDifferenceInMilliseconds < hoursInMs) {
        const minutes: number = Math.floor(timeDifferenceInMilliseconds / minutesInMs);
        ...
        const months: number = Math.floor(timeDifferenceInMilliseconds / monthsInMs);
